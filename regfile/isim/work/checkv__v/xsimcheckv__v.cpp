...
va_list vap)
{
    extern HSim__s6 * createworkMcheckv__v(const char*);
    HSim__s6 *blk = createworkMcheckv__v(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("checkv_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("regd", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMcheckv__v(const char*);
        topvl = (HSim__s5*)createworkMcheckv__v("checkv_v");
