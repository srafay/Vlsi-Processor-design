...
va_list vap)
{
    extern HSim__s6 * createworkMregcheck__v(const char*);
    HSim__s6 *blk = createworkMregcheck__v(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("regcheck_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("regd", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMregcheck__v(const char*);
        topvl = (HSim__s5*)createworkMregcheck__v("regcheck_v");
