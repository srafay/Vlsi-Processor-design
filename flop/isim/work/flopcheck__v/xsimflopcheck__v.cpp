...
va_list vap)
{
    extern HSim__s6 * createworkMflopcheck__v(const char*);
    HSim__s6 *blk = createworkMflopcheck__v(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("flopcheck_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("flopd", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMflopcheck__v(const char*);
        topvl = (HSim__s5*)createworkMflopcheck__v("flopcheck_v");
