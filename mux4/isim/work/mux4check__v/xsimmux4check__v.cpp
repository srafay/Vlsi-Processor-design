...
va_list vap)
{
    extern HSim__s6 * createworkMmux4check__v(const char*);
    HSim__s6 *blk = createworkMmux4check__v(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("mux4check_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("mux4", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMmux4check__v(const char*);
        topvl = (HSim__s5*)createworkMmux4check__v("mux4check_v");
