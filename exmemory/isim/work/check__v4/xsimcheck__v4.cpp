...
va_list vap)
{
    extern HSim__s6 * createworkMcheck__v4(const char*);
    HSim__s6 *blk = createworkMcheck__v4(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMexmemory(const char*);
    HSim__s6 *blk = createworkMexmemory(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("check_v4", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("exmemory", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMcheck__v4(const char*);
        topvl = (HSim__s5*)createworkMcheck__v4("check_v4");
