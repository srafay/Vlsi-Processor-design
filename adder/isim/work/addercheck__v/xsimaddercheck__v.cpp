...
va_list vap)
{
    extern HSim__s6 * createworkMaddercheck__v(const char*);
    HSim__s6 *blk = createworkMaddercheck__v(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("addercheck_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("adder", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMaddercheck__v(const char*);
        topvl = (HSim__s5*)createworkMaddercheck__v("addercheck_v");
