...
va_list vap)
{
    extern HSim__s6 * createworkMandcheck__v(const char*);
    HSim__s6 *blk = createworkMandcheck__v(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("andcheck_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("andop", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMandcheck__v(const char*);
        topvl = (HSim__s5*)createworkMandcheck__v("andcheck_v");
