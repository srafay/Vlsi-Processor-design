...
va_list vap)
{
    extern HSim__s6 * createworkMcondinvcheck__v(const char*);
    HSim__s6 *blk = createworkMcondinvcheck__v(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("condinvcheck_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("mux2d", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("andop", (HSimInstFactoryPtr)IF2);
        (*cfgvh).addVlogModule("condinvd", (HSimInstFactoryPtr)IF3);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF4);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMcondinvcheck__v(const char*);
        topvl = (HSim__s5*)createworkMcondinvcheck__v("condinvcheck_v");
