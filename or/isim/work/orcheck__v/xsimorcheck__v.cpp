...
va_list vap)
{
    extern HSim__s6 * createworkMorcheck__v(const char*);
    HSim__s6 *blk = createworkMorcheck__v(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("orcheck_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("orop", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMorcheck__v(const char*);
        topvl = (HSim__s5*)createworkMorcheck__v("orcheck_v");
