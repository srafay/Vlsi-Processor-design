...
va_list vap)
{
    extern HSim__s6 * createworkMinvertcheck__v(const char*);
    HSim__s6 *blk = createworkMinvertcheck__v(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("invertcheck_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("invertd", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMinvertcheck__v(const char*);
        topvl = (HSim__s5*)createworkMinvertcheck__v("invertcheck_v");
