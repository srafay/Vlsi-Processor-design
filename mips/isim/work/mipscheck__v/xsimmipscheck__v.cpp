...
va_list vap)
{
    extern HSim__s6 * createworkMmipscheck__v(const char*);
    HSim__s6 *blk = createworkMmipscheck__v(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMstatelogic(const char*);
    HSim__s6 *blk = createworkMstatelogic(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMoutputlogic(const char*);
    HSim__s6 *blk = createworkMoutputlogic(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMaludec(const char*);
    HSim__s6 *blk = createworkMaludec(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMcontroller(const char*);
    HSim__s6 *blk = createworkMcontroller(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMmips(const char*);
    HSim__s6 *blk = createworkMmips(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("mipscheck_v", (HSimInstFactoryPtr)IF0);
        ...
        (*cfgvh).addVlogModule("statelogic", (HSimInstFactoryPtr)IF9);
        (*cfgvh).addVlogModule("outputlogic", (HSimInstFactoryPtr)IF10);
        (*cfgvh).addVlogModule("aludec", (HSimInstFactoryPtr)IF11);
        (*cfgvh).addVlogModule("regd", (HSimInstFactoryPtr)IF12);
        (*cfgvh).addVlogModule("mux4", (HSimInstFactoryPtr)IF13);
        (*cfgvh).addVlogModule("mux33", (HSimInstFactoryPtr)IF14);
        (*cfgvh).addVlogModule("mux2d", (HSimInstFactoryPtr)IF15);
        (*cfgvh).addVlogModule("flopenrd", (HSimInstFactoryPtr)IF16);
        (*cfgvh).addVlogModule("flopen", (HSimInstFactoryPtr)IF17);
        (*cfgvh).addVlogModule("flopd", (HSimInstFactoryPtr)IF18);
        (*cfgvh).addVlogModule("alu", (HSimInstFactoryPtr)IF19);
        (*cfgvh).addVlogModule("datav", (HSimInstFactoryPtr)IF20);
        (*cfgvh).addVlogModule("controller", (HSimInstFactoryPtr)IF21);
        (*cfgvh).addVlogModule("mips", (HSimInstFactoryPtr)IF22);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF23);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMmipscheck__v(const char*);
        topvl = (HSim__s5*)createworkMmipscheck__v("mipscheck_v");
